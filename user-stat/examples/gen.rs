...
async fn main() -> Result<()> {
    let pool = PgPool::connect("postgres://postgres:postgres@localhost/stats")
        .await
        .unwrap();
    for i in 1..=500 {
...
async fn raw_insert(users: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
    ...
    sql.push_str(
        "
    INSERT INTO user_stats(email, name, created_at, last_visited_at,
    last_watched_at, recent_watched, viewed_but_not_started, started_but_not_finished,
    finished, last_email_notification, last_in_app_notification, last_sms_notification) VALUES ",
    );
    for user in users {
        sql.push_str(&format!(
            "('{}', '{}', '{}', '{}', '{}', {}::int[], {}::int[], {}::int[], {}::int[], '{}', '{}', '{}'),",
...
async fn bulk_insert(users: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
    ...
    for user in users {
        let query = sqlx::query(
            r#"
            INSERT INTO user_stats(email, name, created_at, last_visited_at, last_watched_at,
            recent_watched, viewed_but_not_started, started_but_not_finished, finished,
            last_email_notification, last_in_app_notification, last_sms_notification
            )
...
impl Dummy<IntList> for Vec<i32> {
    fn dummy_with_rng<R: Rng + ?Sized>(config: &IntList, rng: &mut R) -> Self {
        let (max, start, lent) = (config.0, config.1, config.2);
        let size = rng.gen_range(0..max);
        (0..size)
            .map(|_| rng.gen_range(start..start + lent))
            .collect()
    }
}
